...
func (scte35 *SCTE35) DecodeFromRawBytes(input []byte) (numOfParsedBits int, err error) {
	...
	length := int(scte35.DescriptorLoopLength)
	numOfBitsForDescriptors := int(scte35.DescriptorLoopLength) * 8
	endBitPos := numOfParsedBits + numOfBitsForDescriptors + 16

	for length > 2 && numOfParsedBits < endBitPos {
		spliceDescriptor := &SpliceDescriptor{}
		tmpBytes, _, err = bits.SubBits(input, numOfParsedBits, 0)

		descUsedBits := 0
		descUsedBits, _ = spliceDescriptor.DecodeFromRawBytes(tmpBytes)

		length = length - (descUsedBits / 8)
